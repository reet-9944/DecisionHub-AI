    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      <div style={{ position: 'relative', width: size, height: size }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ color: C, fontSize: size * 0.1, fontWeight: 600 }}>{value >= 75 ? 'High' : value >= 50 ? 'Medium' : 'Low'}</div>
        </div>
      </div>
...
const STEPS = ['Checking your location…','Searching government programs…','Evaluating eligibility…','Finding application steps…','Compiling resources…'];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto 2rem' }}>
        ...
        <div style={{ position: 'absolute', inset: '22%', borderRadius: '50%', background: `linear-gradient(135deg,${C},#a855f7)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🏛️</div>
      </div>
      <div style={{ color: '#fff', fontWeight: 700, fontSize: 17, marginBottom: 8 }}>AI is finding your benefits…</div>
...
const SERVICES = ['Housing Assistance','Food Programs','Healthcare Coverage','Unemployment Benefits','Education Grants','Small Business Support','Disability Services','Child Care Assistance'];

export default function PublicServicesPage() {
  const navigate = useNavigate();
  const [mode, setMode] = useState('input');
  const [form, setForm] = useState({ location: '', serviceNeeded: '', eligibilityDetails: '' });
  const [result, setResult] = useState(null);
  const [error, setError] = useState('');

  const handleAnalyze = async () => {
    if (!form.location || !form.serviceNeeded) { setError('Location and service type are required.'); return; }
    setError(''); setMode('loading');
    try {
      const res = await analyzeWithAI('public', form);
      ...
      <div style={{ background: '#060a0f', borderBottom: `1px solid ${C}12`, padding: '1.5rem 2rem' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div style={{ width: 48, height: 48, borderRadius: 14, background: `${C}18`, border: `1px solid ${C}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🏛️</div>
            <div>
              <h1 style={{ color: '#fff', fontSize: 22, fontWeight: 800, margin: 0 }}>Public Services Navigator</h1>
              <p style={{ color: '#64748b', fontSize: 12, margin: 0 }}>Find government programs & navigate eligibility requirements</p>
            </div>
          </div>
          {mode === 'result' && (
            <button onClick={() => { setMode('input'); setResult(null); }}
              style={{ padding: '8px 18px', borderRadius: 10, background: `${C}12`, border: `1px solid ${C}25`, color: C2, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>↺ New Search</button>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '2rem 1.5rem' }}>
        {mode === 'input' && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: `linear-gradient(135deg,${C}10,rgba(168,85,247,0.04))`, border: `1px solid ${C}18`, borderRadius: 20, padding: '2rem', marginBottom: '1.5rem', textAlign: 'center' }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>🏛️</div>
              <h2 style={{ color: '#fff', fontSize: 22, fontWeight: 800, margin: '0 0 8px' }}>Find Your Benefits</h2>
              <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>AI searches SNAP, Medicaid, Section 8, TANF and hundreds of programs to find what you qualify for</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '1.25rem' }}>
              <Card accent={C} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ color: '#fff', fontWeight: 700, fontSize: 14 }}>📍 Your Location & Need</div>
                <div>
                  <label style={{ display: 'block', color: '#94a3b8', fontSize: 11, fontWeight: 600, marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>City, State</label>
                  <input value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))}
                    placeholder="e.g. Austin, Texas" style={inp}
                    onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.08)'} />
                </div>
                <div>
                  <label style={{ display: 'block', color: '#94a3b8', fontSize: 11, fontWeight: 600, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Service Needed</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {SERVICES.map(s => (
                      <button key={s} onClick={() => setForm(p => ({ ...p, serviceNeeded: s }))}
                        style={{ padding: '5px 12px', borderRadius: 8, border: `1px solid ${form.serviceNeeded === s ? C : 'rgba(255,255,255,0.08)'}`, background: form.serviceNeeded === s ? `${C}18` : '#0d1520', color: form.serviceNeeded === s ? C2 : '#64748b', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                        {s}
                      </button>
                    ))}
                  </div>
              ...
              <Card style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ color: '#fff', fontWeight: 700, fontSize: 14 }}>📋 Eligibility Details</div>
                <div>
                  <label style={{ display: 'block', color: '#94a3b8', fontSize: 11, fontWeight: 600, marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Tell us about your situation</label>
                  <textarea value={form.eligibilityDetails} onChange={e => setForm(p => ({ ...p, eligibilityDetails: e.target.value }))}
                    placeholder="e.g. Single parent, 2 children, monthly income $2,000, currently unemployed, renting…" rows={8}
                    style={{ ...inp, resize: 'vertical' }}
                    onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.08)'} />
                </div>
                <div style={{ padding: '10px 12px', background: `${C}08`, border: `1px solid ${C}15`, borderRadius: 10 }}>
                  <div style={{ color: C2, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>💡 What to include</div>
                  <div style={{ color: '#64748b', fontSize: 11, lineHeight: 1.6 }}>Income level · Household size · Employment status · Age · Disability status · Citizenship</div>
                </div>
              </Card>
            </div>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleAnalyze}
              style={{ marginTop: '1.5rem', width: '100%', padding: 16, borderRadius: 14, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,${C},#a855f7)`, color: '#fff', fontWeight: 800, fontSize: 16, boxShadow: `0 6px 24px ${C}35` }}>
              🏛️ Find My Benefits
            </motion.button>
          </motion.div>
        )}
        ...
        {mode === 'result' && result && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <Card accent={C}>
              <Title icon="🏛️" title="Programs Found" badge={`${result.confidence}% Match`} />
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <Ring value={result.confidence} size={120} stroke={10} label="Eligibility Match" />
              </div>
              <div style={{ padding: '1rem 1.25rem', background: `${C}08`, borderLeft: `3px solid ${C}`, borderRadius: '0 10px 10px 0', marginBottom: '1rem' }}>
                <div style={{ color: '#64748b', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>AI Summary</div>
            ...
            <Card>
              <Title icon="🧠" title="Eligibility Analysis" />
            ...
            <Card accent={C}>
              <Title icon="✅" title="How to Apply" badge={`${result.actions?.length} steps`} />
            ...
            {result.alternatives?.length > 0 && (
              <Card>
                <Title icon="💡" title="Other Programs to Explore" />
            ...
            <div style={{ padding: '1rem', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: 12, display: 'flex', gap: 10 }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: 12, lineHeight: 1.7, margin: 0 }}>
                <strong style={{ color: '#f59e0b' }}>Note:</strong> Always contact your local government office to confirm eligibility. Program availability and requirements may change.
              </p>
            </div>
            <button onClick={() => { setMode('input'); setResult(null); }}
